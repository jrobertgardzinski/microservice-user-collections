Feature: Listing a COLLECTION
  ...
  A COLLECTION belongs to one person and one name: alice's "favourites" is not
  bob's, and not alice's "watchlist". The listing shows the newest save first,
  and without a signed-in identity there is no listing at all.
  ...
  Rule: Without an identity there are no COLLECTIONS
