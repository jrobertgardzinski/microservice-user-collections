 ...
 * have.
 *
 * <p>A port of its own rather than a method on {@link CollectionRepository} or {@link ItemErasure}, and
 * for once the reason is not the usual one about two questions: it is that the question belongs to
 * NEITHER of them. {@link CollectionRepository} is the owner's world and may not see a reserved row;
